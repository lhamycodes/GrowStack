...
<html lang="en">
    <head>
        ...
        <title>GrowStack - Manage Trainers</title>
                                ...
                                <table class="table table-striped">
                                    ...
                                    <tbody>
                                        <?php
                                            ...
                                            $crud = new CRUD();  
                                            $selectParameters = "`accounttype` = 'Trainer' and `status` = 'Active'";
                                            $fetchBooks = $crud->selectRecord('users', $selectParameters);
                                            $jsonFetchBooks = json_decode($fetchBooks);
                                            $fetchStatus = $jsonFetchBooks->{'status'};
                                            $fetchRes = $jsonFetchBooks->{'response'};
                                            if($fetchStatus == 200){
                                                $fetchResponse = $fetchRes->{'response'};
                                                $appLength = count($fetchResponse);
                                                for($i = 0; $i < $appLength; $i++) {
                                                    $index = $i +1;
                                                    $trainersID = $fetchResponse[$i]->{'id'};
                                                    $trainersName = $fetchResponse[$i]->{'name'};
                                                    $trainersEmail = $fetchResponse[$i]->{'emailaddress'};
                                                    $trainersSkill = $fetchResponse[$i]->{'skills'};
                                                    $trainersPhone = $fetchResponse[$i]->{'phone'};

                                                    echo "<tr>
                                                        <td>$index</td>
                                                        <td>$trainersName</td>                                                        
                                                        <td><a href='mailto:$trainersEmail' class='hlink'>$trainersEmail</a></td>
                                                        <td>$trainersSkill</td>";
                                                        echo '<td class="text-center"><a href="" data-id="'.$trainersID.'" data-name="'.$trainersName.'" data-email="'.$trainersEmail.'" data-phone="'.$trainersPhone.'" data-skill="'.$trainersSkill.'" class="editTrainer hlink" data-toggle="modal" data-target="#editTrainerModal">Edit</a>&emsp;<a href="" data-id="'.$trainersID.'" class="deleteTrainer hlink" data-toggle="modal" data-target="#deleteTrainerModal">Delete</a></td>
                                                        </tr>';
                                                }
                                            }
                                            else
    ...
    <div class="modal fade" id="editTrainerModal" tabindex="-1" role="dialog" aria-labelledby="modalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form id="updateTrainersForm">
                    <input type="hidden" name="editTrainerID" id="editTrainerID">
                    <div id="updTrainersMsgBox"></div>
